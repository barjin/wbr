export default class UpdaterFactory {
  static ArrayIdxUpdater<T extends unknown>(
    input: T[],
  ...
  static ArrayIdxRearrange<T extends unknown>(
    input: T[],
    callback: (arg0: T[]) => void,
    options?: { deleteEmpty: boolean },
  ) {
  ...
  static ArrayIdxDeleter<T extends unknown>(
    input: T[],
    callback: (arg0: T[]) => void,
  ) {
    return (idx: number) => () => {
  ...
  static ArrayPusher<T extends unknown>(
    input: T[],
    callback: (arg0: T[]) => void,
  ) {
    return () => (newElement: T) => {
  ...
  static ObjectKeyUpdater<T extends Record<string, unknown>>(
    input: T,
    callback: (arg0: T) => void,
  ) {
    return (oldKey: string) => (newKey: string) => {
      const value = input[oldKey];
      const { [oldKey]: toBeRemoved, ...rest } = input;
      callback({ ...rest, [newKey]: value } as any);
    };
  }

  static ObjectValueUpdater<T extends Record<string, unknown>>(
    input: T,
    callback: (arg0: T) => void,
  ) {
  ...
  static ObjectAddKey<T extends Record<string, unknown>>(
    input: T,
    callback: (arg0: T) => void,
  ) {
  ...
  static ObjectRemoveKey<T extends Record<string, unknown>>(
    input: T,
    callback: (arg0: T) => void,
  ) {
